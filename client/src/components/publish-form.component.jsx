...
const PublishForm = () => {
  ...
  const handlePublish = (e) =>{
    ...
    if(!title.length){
      return toast.error("Write story Title befor publising")
    }
    if(!des.length || des.length > charLength)
      return toast.error(`Write a description about your story within ${charLength} characters to publish`)
    if(!tags.length || tags.length > 10){
      return toast.error(`Write some tags about story within ${tagLimit} taglimit to publish`)
    }
    ...
    <AnimationWrapper >
      <section className='w-screen min-h-screen grid items-center lg:grid-cols-2 py-16 lg:gap-4'>
        ...
        <div className='border-grey lg:border-1 lg:pl-4'>
          <p className='text-dark-grey mb-2 mt-9'>Story Title</p>
          <input type="text" placeholder='Story Title' defaultValue={title} className='input-box pl-4' onChange={handleBlogTitleChange}/>

          <p className='text-dark-grey mb-2 mt-9'>Short description about your Story</p> 
          <textarea
          ...
          <p className='text-dark-grey mb-2 mt-9'>Topics - ( Helps in searching and ranking your Story post )</p>
